	...
	public class ad_ApprovalGivenOnDAO : IDisposable
	{
		...
		public List<ad_ApprovalGivenOn> Get(Int32? id = null)
		{
			try
			{
				...
				ad_ApprovalGivenOnLst = dbExecutor.FetchData<ad_ApprovalGivenOn>(CommandType.StoredProcedure, "wsp_ad_ApprovalGivenOn_Get", colparameters);
		...
		public List<ad_ApprovalGivenOn> GetDynamic(string whereCondition,string orderByExpression)
		{
			try
			{
				...
				ad_ApprovalGivenOnLst = dbExecutor.FetchData<ad_ApprovalGivenOn>(CommandType.StoredProcedure, "wsp_ad_ApprovalGivenOn_GetDynamic", colparameters);
				return ad_ApprovalGivenOnLst;
			}
			catch (Exception ex)
			{
		...
		public string Post(ad_ApprovalGivenOn _ad_ApprovalGivenOn, string transactionType)
		{
			string ret = string.Empty;
			try
			{
				Parameters[] colparameters = new Parameters[4]{
				new Parameters("@paramId", _ad_ApprovalGivenOn.Id, DbType.Int32, ParameterDirection.Input),
				new Parameters("@paramApprovalGivenOn", _ad_ApprovalGivenOn.ApprovalGivenOn, DbType.String, ParameterDirection.Input),
				new Parameters("@paramIsActive", _ad_ApprovalGivenOn.IsActive, DbType.Boolean, ParameterDirection.Input),
				new Parameters("@paramTransactionType", transactionType, DbType.String, ParameterDirection.Input)
				};
				dbExecutor.ManageTransaction(TransactionType.Open);
				ret = dbExecutor.ExecuteScalarString(true, CommandType.StoredProcedure, "wsp_ad_ApprovalGivenOn_Post", colparameters, true);
