...
namespace DbExecutor
{
	public class error_LogDAO //: IDisposible
	{
		...
		public List<error_Log> GetAll(Int64? ErrorId = null)
		{
			try
			{
				List<error_Log> error_LogLst = new List<error_Log>();
				Parameters[] colparameters = new Parameters[1]{
				new Parameters("@ErrorId", ErrorId, DbType.Int32, ParameterDirection.Input)
				};
				error_LogLst = dbExecutor.FetchData<error_Log>(CommandType.StoredProcedure, "error_Log_Get", colparameters);
				return error_LogLst;
			}
			catch (Exception ex)
			{
		...
		public int Add(error_Log _error_Log)
		{
			...
			try
			{
				...
				ret = dbExecutor.ExecuteScalar32(true, CommandType.StoredProcedure, "error_Log_Create", colparameters, true);
		...
		public int Delete(Int64 ErrorId)
		{
			try
			{
				...
				ret = dbExecutor.ExecuteNonQuery(CommandType.StoredProcedure, "error_Log_DeleteById", colparameters, true);
				return ret;
			}
			catch (Exception ex)
			{
